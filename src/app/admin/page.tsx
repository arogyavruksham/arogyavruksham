...
export default function AdminDashboard() {
  ...
  return (
    <div className="space-y-6 max-w-full font-sans text-[#111827] pb-24">
      
      {/* Header & Global Filter */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-5 border-b border-[#E5E7EB] pb-8">
        <div>
          <h1 className="text-2xl md:text-2xl font-black tracking-tighter text-[#111827]">Dashboard</h1>
          <p className="text-sm md:text-base text-[#6B7280] mt-2 max-w-[65ch]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white p-5 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] flex flex-col justify-between h-48 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] group">
          ...
          <div>
            <p className="text-4xl font-black tracking-tighter text-[#111827] mb-1">₹{metrics.totalSales.toLocaleString('en-IN')}</p>
            <span className="inline-flex items-center text-[10px] font-bold text-[#9CA3AF] tracking-wide uppercase">
        ...
        <div className="bg-white p-5 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] flex flex-col justify-between h-48 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] group">
          ...
          <div>
            <p className="text-4xl font-black tracking-tighter text-[#111827] mb-1">{metrics.totalOrders}</p>
            <span className="inline-flex items-center text-[10px] font-bold text-[#9CA3AF] tracking-wide uppercase">
        ...
        <div className="bg-white p-5 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] flex flex-col justify-between h-48 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] group">
          ...
          <div>
            <p className="text-4xl font-black tracking-tighter text-[#111827] mb-1">{metrics.totalCustomers}</p>
            <span className="inline-flex items-center text-[10px] font-bold text-[#9CA3AF] tracking-wide uppercase">
        ...
        <div className="bg-white p-5 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] flex flex-col justify-between h-48 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-700 ease-[cubic-bezier(0.32,0.72,0,1)] group">
          ...
          <div>
            <p className="text-4xl font-black tracking-tighter text-[#111827] mb-1">{metrics.pendingDelivery}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white p-5 md:p-10 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] lg:col-span-2 flex flex-col">
          ...
          <div className="flex flex-wrap gap-6 sm:gap-10 mb-10 pb-8 border-b border-[#E5E7EB]">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#9CA3AF] mb-2">Total Income</p>
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-black tracking-tighter text-[#111827]">₹{chartTotals.income.toLocaleString('en-IN')}</p>
              </div>
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#9CA3AF] mb-2">Est. Expenses</p>
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-black tracking-tighter text-[#9CA3AF]">₹{chartTotals.expenses.toLocaleString('en-IN')}</p>
              </div>
            </div>
            <div>
              <p className="text-[11px] font-bold uppercase tracking-widest text-[#111827] mb-2">Net Balance</p>
              <div className="flex items-baseline gap-2">
                <p className="text-2xl font-black tracking-tighter text-[#059669]">₹{chartTotals.balance.toLocaleString('en-IN')}</p>
              </div>
            </div>
          </div>
            ...
            <div className="w-full mt-8 space-y-5 bg-[#F9FAFB] p-6 rounded-lg border border-[#E5E7EB]">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[11px] font-bold uppercase tracking-widest text-[#6B7280]">Daily Mark</span>
                </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {topProducts.map((product) => (
              <div key={product.id} className="group flex flex-col p-4 bg-[#F3F4F6] border-0 shadow-sm text-[#374151] rounded-lg hover:bg-[#059669] shadow-sm border-0/5 transition-colors cursor-pointer">
                ...
                <h4 className="font-bold text-[#111827] text-sm truncate mb-2">{product.title || product.name}</h4>
                <div className="flex items-center justify-between mt-auto">
                  <span className="text-sm font-black tracking-tight text-[#111827]">₹{product.price}</span>
                  <span className="text-[10px] uppercase font-bold tracking-widest text-[#6B7280]">{product.sales_count} Sold</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-white p-5 md:p-10 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB]">
          ...
          <div className="space-y-4">
            {activeCoupons.length === 0 ? (
              <p className="text-sm text-[#6B7280] italic">No active campaigns.</p>
      ...
      <div className="bg-white rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#E5E7EB] overflow-hidden">
        ...
        <div className="overflow-x-auto p-4 md:p-6">
          <table className="w-full text-left border-collapse min-w-[800px] whitespace-nowrap">
            <thead>
              <tr className="border-b border-[#E5E7EB] text-[10px] uppercase tracking-widest text-[#9CA3AF] font-bold">
            ...
            <tbody className="text-sm">
              {recentOrders.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-10 text-center text-[#6B7280] font-medium">
                  ...
                  <tr key={order.id} className="group hover:bg-[#F3F4F6] transition-colors rounded-lg">
                    ...
                    <td className="p-4 align-middle">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-full bg-white border border-[#E5E7EB] flex items-center justify-center font-black text-xs text-[#111827] shadow-[0_2px_8px_rgba(0,0,0,0.02)] shrink-0">
                          {avatarLetter}
                        </div>
                        <div>
                      ...
                      {order.shipping_address ? (
                        <div className="flex items-center gap-3 w-full min-w-[200px] whitespace-normal">
                          <div className="w-8 h-8 rounded-full bg-white border border-[#E5E7EB] flex items-center justify-center shrink-0">
                            <MapPin className="w-3 h-3 text-[#9CA3AF]" strokeWidth={1.5} />
                          </div>
                          <div>
                            <span className="font-bold text-[#111827] text-sm block">{order.shipping_address.name}</span>
                        ...
                        <span className="text-xs text-[#9CA3AF] font-medium">Digital/No Info</span>
                      )}
                    </td>
                    <td className="p-4 align-middle">
                      <span className="font-black tracking-tight text-[#111827] text-base">
