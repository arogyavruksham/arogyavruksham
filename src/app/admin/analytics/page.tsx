...
export default function AnalyticsPage() {
  ...
  const selectedDayStatusStats = selectedDayData ? [
    ...
    { name: 'Pending', value: selectedDayData.orders.filter(o => o.status === 'pending').length, color: '#FF6B26' },
  ...
  return (
    <div className="space-y-6 max-w-full text-gray-800 pb-16 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-gray-200 pb-4">
        ...
        {/* Date Range Selector matching TagMango design */}
        <button className="flex items-center gap-3 px-4 py-2 bg-white border border-gray-200 rounded-xl shadow-2xs text-sm font-semibold hover:border-gray-300 transition-colors shrink-0 cursor-pointer text-gray-700">
          <span className="text-[#3B82F6]">2026-07-29</span>
          <span className="text-gray-300">&mdash;</span>
          <span className="text-[#3B82F6]">2026-08-05</span>
      ...
      <div className="flex items-center gap-6 border-b border-gray-200 overflow-x-auto text-sm no-scrollbar">
        {[
          ...
          <button
            ...
            className={`pb-3 font-semibold whitespace-nowrap transition-all border-b-2 cursor-pointer ${
              activeTab === tab 
                ? 'text-[#FF6B26] border-[#FF6B26] font-black' 
      ...
      <div className="pt-1">
        <div className="inline-block relative">
          <select 
            ...
            className="appearance-none bg-white border border-gray-200 text-gray-700 text-sm font-semibold rounded-xl pl-4 pr-10 py-2 shadow-2xs outline-none focus:border-[#FF6B26] focus:ring-1 focus:ring-[#FF6B26] cursor-pointer"
          >
      ...
      {/* Metric Cards in vibrant TagMango orange style */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        
        {/* Card 1: Net Profit */}
        <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-2xs flex flex-col justify-between hover:shadow-xs transition-shadow">
          <div className="flex items-center justify-between text-gray-500 text-sm font-medium">
            <span>Total Net Profit</span>
            <span title="Calculated revenue minus total product acquisition cost" className="cursor-help text-gray-400 hover:text-gray-600">
              <Info className="w-4 h-4" />
            </span>
          </div>
          <div className="mt-4">
            <h3 className="text-2xl md:text-3xl font-black text-[#FF6B26]">
              ₹{stats.totalProfit.toLocaleString('en-IN')}
            </h3>
            <span className="text-xs text-green-600 font-semibold mt-1 block">Live profit tracking</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-2xs flex flex-col justify-between hover:shadow-xs transition-shadow">
          <div className="flex items-center justify-between text-gray-500 text-sm font-medium">
            ...
            <span title="Orders successfully delivered to customer" className="cursor-help text-gray-400 hover:text-gray-600">
              <Info className="w-4 h-4" />
            </span>
          </div>
          <div className="mt-4">
            ...
            <span className="text-xs text-gray-400 font-semibold mt-1 block">Fully delivered</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-2xs flex flex-col justify-between hover:shadow-xs transition-shadow">
          <div className="flex items-center justify-between text-gray-500 text-sm font-medium">
            ...
            <span title="Orders currently dispatched and in transit" className="cursor-help text-gray-400 hover:text-gray-600">
              <Info className="w-4 h-4" />
            </span>
          </div>
          <div className="mt-4">
            ...
            <span className="text-xs text-blue-600 font-semibold mt-1 block">In active transit</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-5 border border-gray-200/80 shadow-2xs flex flex-col justify-between hover:shadow-xs transition-shadow">
          <div className="flex items-center justify-between text-gray-500 text-sm font-medium">
            ...
            <span title="Total transactions utilizing promotional discount vouchers" className="cursor-help text-gray-400 hover:text-gray-600">
              <Info className="w-4 h-4" />
            </span>
          </div>
          <div className="mt-4">
            ...
            <span className="text-xs text-purple-600 font-semibold mt-1 block">Promotions active</span>
          </div>
        </div>

      </div>

      {/* Info Notice Banner matching TagMango blue alert bar */}
      <div className="bg-[#EFF6FF] border border-blue-100 rounded-xl p-3 px-4 text-xs font-semibold text-[#1E40AF] flex items-center gap-2.5 shadow-2xs">
        <Info className="w-4 h-4 text-[#3B82F6] shrink-0" />
        <span>Visibility is limited to data generated on or after <strong className="font-black text-gray-900 underline">November 22, 2023</strong>.</span>
      </div>

      {/* Global Peak Order Times Graph */}
      <div className="bg-white p-6 rounded-2xl shadow-2xs border border-gray-200/80 mt-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-2">
          <div>
            <h2 className="text-lg font-black text-gray-900 flex items-center gap-2">
              <Clock className="w-5 h-5 text-[#FF6B26]" /> Overall Order Time Analysis
            </h2>
            <p className="text-xs text-gray-500 mt-0.5">Analyses order placement times across the entire store history</p>
          </div>
          <span className="px-3 py-1 bg-[#FFF6F0] text-[#FF6B26] border border-[#FFE1D1] text-xs font-bold rounded-full">24-Hour Breakdown</span>
        </div>
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={overallTimeStats} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Area 
                ...
                dot={{ r: 3, fill: '#fff', stroke: '#FF6B26', strokeWidth: 2 }} 
      ...
      <div className="bg-white rounded-2xl shadow-2xs border border-gray-200/80 overflow-hidden">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <div>
            <h2 className="text-lg font-black text-gray-900 flex items-center gap-2">
              <Calendar className="w-5 h-5 text-[#FF6B26]" /> Daily Analytics & Performance
            </h2>
              ...
              {dailyStats.length === 0 ? (
                ...
                >
                  <td className="p-4 pl-6 font-bold text-gray-900 flex items-center gap-2">
                    <Calendar className="w-4 h-4 text-gray-400 group-hover:text-[#FF6B26] transition-colors" />
                    {day.formattedDate}
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    <span className="px-2.5 py-1 bg-green-50 text-green-700 border border-green-200/60 rounded-lg font-bold text-xs flex items-center gap-1 w-fit">
                      <CheckCircle2 className="w-3.5 h-3.5" /> {day.deliveredOrders}
                    </span>
                  </td>
                  <td className="p-4">
                    <span className="px-2.5 py-1 bg-orange-50 text-orange-700 border border-orange-200/60 rounded-lg font-bold text-xs flex items-center gap-1 w-fit">
                  ...
                  <td className="p-4 font-black text-[#FF6B26]">
                    ₹{day.profit.toLocaleString('en-IN')}
                  </td>
                  <td className="p-4 text-right pr-6">
                    <button 
                      onClick={(e) => { e.stopPropagation(); setSelectedDateStr(day.dateStr); }}
                      className="inline-flex items-center gap-1 px-3 py-1.5 rounded-xl bg-[#FFF6F0] text-[#FF6B26] border border-[#FFE1D1] hover:bg-[#FF6B26] hover:text-white font-bold text-xs transition-all shadow-2xs"
                    >
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden flex flex-col border border-gray-200">
            ...
            <div className="p-6 px-8 border-b border-gray-200 flex justify-between items-center bg-gray-50/80 sticky top-0 z-10">
              <div>
                <span className="text-xs font-black uppercase tracking-wider text-[#FF6B26] block mb-1">Daily Deep-Dive Analytics</span>
            ...
            <div className="p-8 overflow-y-auto space-y-8 flex-1">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="bg-green-50/60 p-4 rounded-2xl border border-green-100">
                  <p className="text-xs font-bold text-green-700 uppercase">Delivered</p>
                ...
                <div className="bg-[#FFF6F0] p-4 rounded-2xl border border-[#FFE1D1]">
                  <p className="text-xs font-black text-[#FF6B26] uppercase">Net Day Profit</p>
                  <p className="text-2xl font-black text-[#FF6B26] mt-1">₹{selectedDayData.profit.toLocaleString('en-IN')}</p>
                </div>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                <div className="lg:col-span-2 bg-white p-6 rounded-2xl border border-gray-200 shadow-2xs flex flex-col">
                  <h4 className="font-bold text-gray-900 mb-1 flex items-center gap-2">
                    <Clock className="w-4 h-4 text-[#FF6B26]" /> Order Time Graph ({selectedDayData.formattedDate})
                  </h4>
                  ...
                  <div className="h-60 w-full min-h-[240px] flex-1">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={selectedDayTimeStats} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        />
                        <Bar dataKey="orders" fill="#FF6B26" radius={[6, 6, 0, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>

                <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-2xs flex flex-col">
                ...
                <div className="p-5 bg-gray-50/60 border-b border-gray-100 flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-gray-900 flex items-center gap-2 text-sm">
                      <ShoppingBag className="w-4 h-4 text-[#FF6B26]" /> All Order Logs for {selectedDayData.formattedDate}
                    </h4>
                          ...
                          >
                            ...
                            <td className="p-3.5">
                              <span className={`px-2 py-0.5 rounded-full font-bold uppercase tracking-wider text-[10px] ${
                                ...
                                order.status === 'pending' ? 'bg-orange-50 text-[#FF6B26]' :
                            ...
                            <td className="p-3.5 text-right pr-5">
                              <span className="inline-flex items-center gap-1 text-[#FF6B26] font-bold group-hover:translate-x-0.5 transition-transform">
            ...
            <div className="p-4 px-8 border-t border-gray-100 bg-gray-50 flex justify-end">
              <button 
                onClick={() => setSelectedDateStr(null)}
                className="px-6 py-2.5 bg-gray-900 text-white font-bold text-sm rounded-xl hover:bg-gray-800 transition-colors shadow-2xs"
              >
                Close Deep-Dive
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-xs flex justify-end animate-in fade-in duration-200">
          <div className="w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gray-50/50">
              <div>
                <span className="text-[10px] font-black uppercase tracking-wider text-[#FF6B26] block">Order Inspection</span>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <User className="w-4 h-4 text-[#FF6B26]" /> Customer Information
                </h3>
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <Package className="w-4 h-4 text-[#FF6B26]" /> Product Information
                </h3>
                <div className="space-y-3">
                  {selectedOrder.order_items?.map((item: any, idx: number) => (
                    <div key={idx} className="flex gap-4 p-3 bg-gray-50/60 rounded-2xl border border-gray-100 items-center">
                      {item.products?.image_url ? (
                        <div className="w-14 h-14 rounded-xl bg-white flex items-center justify-center border border-gray-100 shrink-0 overflow-hidden shadow-2xs">
                          <img src={item.products.image_url} alt={item.products.title} className="w-full h-full object-contain mix-blend-multiply p-1" />
                        </div>
                      ) : (
                        <div className="w-14 h-14 rounded-xl bg-white flex items-center justify-center border border-gray-100 shrink-0 shadow-2xs">
              ...
              <div className="space-y-3">
                <h3 className="text-xs font-bold text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <CreditCard className="w-4 h-4 text-[#FF6B26]" /> Payment Information
                </h3>
              ...
              <div className="space-y-3 pb-8">
                <h3 className="text-xs font-bold text-gray-400 flex items-center gap-2 uppercase tracking-wider">
                  <Truck className="w-4 h-4 text-[#FF6B26]" /> Shipping Status
                </h3>
                  ...
                  {[
                    ...
                    return (
                      <div key={status.id} className="relative z-10 flex gap-4 items-start">
                        <div className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 mt-0.5 border-2 bg-white ${isCompleted ? 'border-[#FF6B26]' : 'border-gray-300'}`}>
                          {isCompleted ? <div className="w-2.5 h-2.5 bg-[#FF6B26] rounded-full"></div> : null}
                        </div>
                        <div>
            ...
            <div className="p-4 px-6 border-t border-gray-100 bg-gray-50 flex justify-end">
              <button 
                ...
                className="px-6 py-2.5 bg-gray-900 text-white font-bold text-sm rounded-xl hover:bg-gray-800 transition-colors shadow-2xs w-full"
              >
