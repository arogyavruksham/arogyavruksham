    ...
    <div className="space-y-6 text-stone-800 font-sans pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-black text-stone-800 tracking-tight flex items-center gap-2">
            <Bot className="w-8 h-8 text-stone-800" />
            Autonomous Agent Insights
          </h1>
          <p className="text-sm font-semibold text-stone-500 mt-1">Live monitoring and intelligent alerts for your store.</p>
        </div>
        <div className="flex items-center gap-2 bg-[#F9F9F8] text-[#1B4332] px-4 py-2 border border-emerald-200 rounded-xl shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-2xs p-6 space-y-4 relative overflow-hidden">
            {summaryLoading && (
              <div className="absolute top-0 left-0 w-full h-1 bg-emerald-100 overflow-hidden">
                <div className="h-full bg-[#1B4332] animate-[progress_1.5s_ease-in-out_infinite] w-1/2 rounded-full"></div>
              </div>
            )}
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-bold text-stone-800">Today's Traffic</h3>
            </div>
            {loading ? (
              <div className="flex justify-center p-4"><Loader2 className="w-6 h-6 animate-spin text-stone-400" /></div>
            ) : (
            ...
            <div className="pt-4 mt-4 border-t border-gray-100 flex items-center justify-between text-xs text-stone-500 font-medium">
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-2xs p-6 min-h-[400px] flex flex-col">
            <div className="flex justify-between items-center mb-6">
              <h3 className="font-bold text-stone-800 flex items-center gap-2">
                <AlertCircle className="w-5 h-5 text-stone-800" />
                Live Agent Report
              </h3>
              {summaryLoading && <span className="text-xs font-bold text-stone-800 animate-pulse bg-[#F9F9F8] px-3 py-1 rounded-full">Analyzing...</span>}
            </div>
            
            {error ? (
              <div className="bg-red-50 text-red-600 p-4 rounded-xl text-sm font-semibold">{error}</div>
            ) : summary ? (
              <div className="prose prose-sm max-w-none text-stone-700 space-y-4">
                {summary.split('\n').map((line, i) => {
                  if (line.startsWith('## ')) return <h2 key={i} className="text-lg font-black text-stone-800 mt-4 mb-2">{line.replace('## ', '')}</h2>
