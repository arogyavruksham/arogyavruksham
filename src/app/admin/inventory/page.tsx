  ...
  return (
    <div className="space-y-6 text-stone-800 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex bg-white border border-stone-200 p-1 rounded-xl shadow-2xs w-full sm:w-auto overflow-x-auto">
          <button 
            onClick={() => setActiveTab('LOW_STOCK')}
            className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-lg transition-all flex items-center gap-2 cursor-pointer ${activeTab === 'LOW_STOCK' ? 'bg-amber-50 text-amber-700 border border-amber-200 shadow-2xs' : 'text-stone-600 hover:text-stone-800 hover:bg-[#F9F9F8]'}`}
          >
          ...
          <button 
            ...
            className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-lg transition-all flex items-center gap-2 cursor-pointer ${activeTab === 'OUT_OF_STOCK' ? 'bg-amber-50 text-amber-700 border border-amber-200 shadow-2xs' : 'text-stone-600 hover:text-stone-800 hover:bg-[#F9F9F8]'}`}
          >
          ...
          <button 
            ...
            className={`px-4 py-2 text-xs font-black uppercase tracking-wider rounded-lg transition-all flex items-center gap-2 cursor-pointer ${activeTab === 'FULL_STOCK' ? 'bg-amber-50 text-amber-700 border border-amber-200 shadow-2xs' : 'text-stone-600 hover:text-stone-800 hover:bg-[#F9F9F8]'}`}
          >
      ...
      <div className="bg-white rounded-2xl border border-stone-200 shadow-2xs overflow-hidden">
        {loading ? (
          <div className="flex flex-col items-center justify-center p-14 gap-2 text-stone-500">
            <Loader2 className="w-8 h-8 animate-spin text-stone-800" />
            <span className="text-xs font-bold text-stone-700">Checking inventory logs...</span>
          </div>
        ) : getActiveList().length === 0 ? (
          <div className="p-14 text-center text-stone-400 font-medium italic">
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap min-w-[600px]">
              ...
              <tbody className="text-sm divide-y divide-gray-100 font-medium">
                {getActiveList().map(product => (
                  <tr key={product.id} className="hover:bg-[#F9F9F8]/80 transition-colors">
                    <td className="p-4 pl-6">
                      <input type="checkbox" className="rounded border-stone-300 text-stone-800 focus:ring-[#1B4332] cursor-pointer" />
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-3.5">
                        <div className="w-11 h-11 rounded-xl bg-[#F9F9F8] overflow-hidden shrink-0 border border-gray-100 p-1 flex items-center justify-center">
                          {product.image_url ? (
                            <img src={product.image_url} alt={product.title} className="w-full h-full object-contain mix-blend-multiply" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-stone-400 font-bold text-[10px] bg-stone-100 rounded-lg">N/A</div>
                          )}
                        </div>
                        <div>
                          <p className="font-bold text-stone-800 text-sm leading-snug">{product.title}</p>
                          <p className="text-xs text-stone-400 font-mono mt-0.5">{product.id.split('-')[0].toLowerCase()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-4 text-stone-600 font-semibold">{product.category}</td>
                    <td className="p-4">
                      <input
                        type="number"
                        min="0"
                        ...
                        className="w-24 px-3 py-1.5 border border-stone-200 rounded-xl font-black text-stone-800 text-sm outline-none focus:border-[#1B4332]"
                      />
                    </td>
                    <td className="p-4">
                      <button
