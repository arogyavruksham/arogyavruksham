    ...
    <div className="space-y-6 text-[#111827] font-sans pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 rounded-xl border border-[#D1D5DB] text-[#374151] font-bold hover:bg-[#F9FAFB] transition-colors text-xs flex items-center gap-1.5 shadow-2xs cursor-pointer"
        >
      ...
      <div className="bg-white p-6 rounded-2xl border border-[#E5E7EB] shadow-2xs space-y-6">
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-xl border border-[#E5E7EB] overflow-hidden bg-[#F9FAFB] p-3 shadow-inner">
            {formData.is_active ? (
              <div 
        ...
        <div className="space-y-2">
          ...
          <div className="rounded-xl border border-[#E5E7EB] overflow-hidden bg-[#F9FAFB] p-3 max-w-sm shadow-inner">
            {formData.is_active ? (
              <div 
                style={{ backgroundColor: formData.bg_color, color: formData.text_color }}
      ...
      {/* Configuration Form */}
      <form onSubmit={handleSubmit} className="bg-white p-6 md:p-5 rounded-2xl border border-[#E5E7EB] shadow-2xs space-y-5">
        
        {/* Toggle Switch - Monochrome */}
        <div className="flex items-center justify-between p-3 bg-[#F9FAFB] rounded-2xl border border-[#E5E7EB]">
          <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input 
                ...
                className="w-full px-4 py-2.5 rounded-xl border border-[#D1D5DB] focus:outline-none focus:border-[#059669] focus:ring-1 focus:ring-[#059669] font-semibold text-sm transition-all bg-white"
              />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
              {COLOR_SWATCHES.map((swatch) => (
                <button
                  ...
                  className={`py-2.5 px-3 rounded-xl font-bold text-xs shadow-2xs transition-all cursor-pointer border ${formData.bg_color === swatch.bg ? 'ring-2 ring-black ring-offset-2 font-black scale-105' : 'border-[#D1D5DB] opacity-90 hover:opacity-100'}`}
                >
                  {swatch.label}
                </button>
              ))}
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <input 
                  ...
                  className="w-11 h-11 rounded-xl border border-[#D1D5DB] cursor-pointer shrink-0 p-0.5 bg-white shadow-2xs"
                />
                <input 
                  type="text" 
                  value={formData.bg_color}
                  onChange={e => handleChange('bg_color', e.target.value)}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-[#D1D5DB] font-mono text-sm font-bold text-[#111827] uppercase"
                />
              </div>
            </div>
        ...
        <div className="pt-4 border-t border-[#E5E7EB] flex justify-end">
          <button
            type="submit"
            disabled={isSaving}
            className="px-8 py-3.5 bg-[#059669] hover:bg-gray-900 disabled:bg-gray-400 text-white font-black text-xs uppercase tracking-wider rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-all duration-200 flex items-center gap-2 cursor-pointer"
          >
